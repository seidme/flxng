...
export class ColumnComponent implements OnInit, AfterContentInit {
  ...
  constructor(@Inject(forwardRef(() => TreetableComponent)) private _dtcRef: TreetableComponent) {}
  ...
  private _collectTemplateRefs(): void {
    this.templateList.toArray().forEach((t: TemplateDirective) => {
      if (!ColumnTemplates[t.type]) {
        console.warn(
          `Unknown template type: ${t.type}. Possible value/s: ${mapToIterable(ColumnTemplates).join(', ')}.`
        );
        return;
      }
